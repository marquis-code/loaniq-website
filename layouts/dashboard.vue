...
<main>
    ...
    <div class="sticky  top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-[#FFFFFF] px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
      ...
      <div class="flex  flex-1 gap-x-4 self-stretch lg:gap-x-6">
        ...
        <div class="flex items-center gap-x-4 lg:gap-x-6">
          <button @click="showNotification = true" type="button" class="-m-2.5 p-2.5 text-gray-400 hover:text-gray-500">
          ...
          <div class="relative">
            <button type="button" class="-m-1.5 flex items-center p-1.5" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
              ...
              <img v-if="profileInfoObj?.profile?.avatar" :src="profileInfoObj?.profile?.avatar" class="h-10 w-10 rounded-full bg-gray-50"  />
              <img v-else class="h-10 w-10 rounded-full bg-gray-50" :src="avatarImage" alt="">
              <span class="hidden lg:flex lg:items-center">
                <span class="ml-2 text-sm font-semibold leading-6 text-gray-900" aria-hidden="true">{{ `${user?.firstName} ${user?.lastName}` || 'Nil' }}</span>
              </span>
            </button>
          </div>
        </div>
  ...
  <CoreNotificationDrawer @close="showNotification = false" :show="showNotification" />
</main>
</template>
