<template>
  ...
  const getRate = (amount: number, tenor: number): number => {
    ...
    // Apply monthly payment adjustment
    rate -= 1.0
    
    return Math.max(rate, 0) // Ensure rate doesn't go negative
  }
  
  // Calculate investment returns based on your company's methodology
  const calculateInvestmentReturns = (
    amount: number,
    frequency: string,
    tenorDays: number
  ): { totalInvested: number; finalAmount: number; totalReturns: number } => {
    const annualRate = getRate(amount, tenorDays) / 100
    ...
    switch (frequency) {
      ...
      case 'Yearly':
        ...
        break
      default:
        periodsInYear = 12 // Default to monthly
    }
  
    const totalPeriods = Math.floor((tenorDays / 365) * periodsInYear)
    const periodRate = annualRate / periodsInYear
    const periodAmount = amount / periodsInYear
  
    // Calculate for each deposit
    for (let period = 0; period < totalPeriods; period++) {
      const remainingPeriods = totalPeriods - period
      const futureValue = periodAmount * Math.pow(1 + periodRate, remainingPeriods)
  ...
  const bankComparison = computed(() => {
    ...
    const { totalInvested } = calculatedResults.value
    return totalInvested * bankRate * (selectedTenor.value / 365)
  })
