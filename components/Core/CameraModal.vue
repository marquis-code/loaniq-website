<!-- <template>
  <div class="flex flex-col items-center justify-center p-4">
    ...
    <div v-if="isPhotoTaken && isCameraOpen" class="camera-download mt-4">
      ...
      <button @click="livelinessCheck" :disabled="loading" cclass="button disabled:cursor-not-allowed disabled:opacity-25 bg-black text-white px-6 py-2 rounded-lg">{{  loading ? 'processing' : 'Proceed to perform liveliness check' }}</button>
    </div>
  </div>
</template> 
...
const onFileSelected = async (event: Event) => {
  ...
  if (file) {
    ...
    if (uploadResponse.value?.secure_url) {
      await livelinessCheck(uploadResponse.value?.secure_url);
    }
  }
};
